...
public class ReferralBillingBridgeService {
    ...
    @Transactional
    public void onFirstPaidSubscriptionVerified(Long inviteeUserId,
        ...
        claimRepository.findByInviteeUserId(inviteeUserId).ifPresent(claim -> {
            if (!ReferralClaimStatus.PENDING_SUBSCRIPTION.name().equals(claim.getStatus())) return;
            if (pending) {
    ...
    @Transactional
    public void markRefundedOrRevoked(String purchaseTokenHash, boolean chargeback, Instant detectedAtUtc) {
