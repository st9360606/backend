...
-- Purpose:
-- 1. Store user membership entitlement state.
-- 2. Support Google Play subscription verification.
-- 3. Support Google Play defer / re-verification flows.
-- 4. Support internal rewards such as referral reward.
-- 5. Preserve billing traceability without storing raw purchase token in plaintext.
-- ============================================================
...
(
    ...
    -- OK / GRACE / ON_HOLD / EXPIRED / REVOKED / PENDING
    -- PENDING_PURCHASE_CANCELED / UNKNOWN / PAUSED
    ...
    UNIQUE KEY uk_entitlements_purchase_token_hash
        (purchase_token_hash),

    INDEX idx_entitlements_user
        (user_id, status, valid_to_utc),

    INDEX idx_entitlements_linked_purchase_token_hash
        (linked_purchase_token_hash),

    INDEX idx_entitlements_source_state
        (source, subscription_state),

    INDEX idx_entitlements_reverify
        (source, status, last_google_verified_at_utc),

    INDEX idx_entitlements_payment_state
        (payment_state, status, valid_to_utc)
